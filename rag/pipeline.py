...
--------------
  ...
  VectorStore       – ChromaDB wrapper; add_documents() for ingestion
  RAGRetriever      – query the store, re-rank, format context for LLM
  get_remote_llm    – helper to connect to Colab-hosted Ollama server
  ask()             – one-call RAG: retrieve → format → generate → return
  load_env()        – lightweight .env loader (no python-dotenv dependency)
"""
...
def get_remote_llm(
    ...
    """
    Helper to instantiate a ChatOllama pointing to your Google Colab Tunnel.

    Parameters
    ----------
    base_url   : Tunnel URL from Colab (e.g. 'https://xxx.a.pinggy.link').
                 Defaults to OLLAMA_BASE_URL environment variable if not passed.
    model_name : Ollama model name running in Colab.
    """
    from langchain_ollama import ChatOllama

    load_env()
    target_url = base_url or os.getenv("OLLAMA_BASE_URL")

    if not target_url:
        raise ValueError(
            "No base_url provided! Set OLLAMA_BASE_URL in your .env file "
            "or pass base_url explicitly to get_remote_llm()."
        )

    logger.info("Connecting local pipeline to remote Colab LLM at %s", target_url)
